        ...
        <div className="flex gap-3 overflow-x-auto no-scrollbar py-1">
          <Link href={`/store/${storeSlug}/menu`} prefetch={false} className="flex-shrink-0 w-[200px] md:w-[240px] group">
            <div className="relative h-[160px] md:h-[180px] rounded-2xl overflow-hidden bg-chocolate shadow-sm">
              ...
              <span className="absolute bottom-3 left-3.5 right-3 text-white font-serif font-bold text-sm drop-shadow-md">View All Menu</span>
            </div>
          </Link>
          {occasions.map((occ) => (
            <Link key={occ.id} href={`/cakes/${occ.slug}`} prefetch={false} className="flex-shrink-0 w-[200px] md:w-[240px] group">
              <div className="relative h-[160px] md:h-[180px] rounded-2xl overflow-hidden bg-chocolate shadow-sm">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
                <span className="absolute bottom-3 left-3.5 right-3 text-white font-serif font-bold text-sm drop-shadow-md">{occ.name}</span>
              </div>
            </Link>
          ))}
          {themes.map((t) => (
            <Link key={t.id} href={`/themes/${t.slug}`} prefetch={false} className="flex-shrink-0 w-[200px] md:w-[240px] group">
              <div className="relative h-[160px] md:h-[180px] rounded-2xl overflow-hidden bg-chocolate shadow-sm">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
                <span className="absolute bottom-3 left-3.5 right-3 text-white font-serif font-bold text-sm drop-shadow-md">{t.name}</span>
              </div>
            </Link>
          ))}
        </div>
